#ifndef NS_GRIDLOGGER_HH
#define NS_GRIDLOGGER_HH
#include <sys/types.h>
#include <sys/uio.h>
#include <unistd.h>
#include <fcntl.h>
...
class NetSimGridLogger : public GridGenericLogger {
  ...
  bool check_space(size_t needed) {
    ...
    if (avail < needed) {
      click_chatter("NetSimGridLogger %s: log buffer is too small.  total buf size: %u, needed at least %u",
  ...
  void write_buf() {
    ...
    if (res < 0)
      click_chatter("NetSimGridLogger %s: error writing log buffer: %s",
		    name().c_str(), strerror(errno));
    else if (res != (int) _bufptr)
      click_chatter("NetSimGridLogger %s: bad write to log buffer, had %u bytes in buffer but wrote %d bytes",
...
public:
  ...
  const char *class_name() const { return "NetSimGridLogger"; }
...
public:
  ...
  void log_added_route(DSDVRouteTable::RTEntry r, reason_t why)
  {
    // simple logic (that may fail) to determine if a route change is due to a better metric and not just seq updated
    StringAccum sa;
    sa << "{"; // start object
    sa << "\"type\":\"route\",";
    sa << "\"time\":" << (unsigned) Timestamp::now ().msecval () << ",";
    sa << "\"dest\":\"" << r.dest_ip.unparse().c_str() << "\",";
    sa << "\"next\":\"" << r.next_hop_ip.unparse().c_str() << "\",";
    sa << "\"metr\":" << r.metric.val() << ",";
    sa << "\"seqn\":" << r.seq_no() << ",";
    sa << "\"resn\":" << why << "";
    sa << "},"; // stop object
    m_jsonStream << sa.c_str();
  }
  void LogMetricUpdate (DSDVRouteTable::RTEntry &r)
  {
    StringAccum sa;
    sa << "{"; // start object
    sa << "\"type\":\"metric\",";
  ...
  void log_route_dump(const Vector<DSDVRouteTable::RTEntry> &rt, const Timestamp &when)
  {
    //TODO - what format is this? just writing text for now
    StringAccum sa;
    sa << "\n<dump time='" << when.timeval() << "'>\n";
    for (int i = 0; i < rt.size(); i++)
      {
        const DSDVRouteTable::RTEntry &r = rt[i];
        sa << r.dump();
      }
    sa << "</dump>\n";
    if (m_logStream.is_open())
      {
        m_logStream << sa.c_str();
      }
  }
